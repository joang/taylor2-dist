...
\date{February 13, 2024} % version 2.2
% \date{May 26, 2023} % version 2.1
% \date{November 28, 2022} % version 2.0.2
% \date{November 7, 2022} % version 2.0.1
% \date{September 9, 2022} % version 2.0.0
...
this case the name is {\tt taylor\symbol{95}step\symbol{95}lrnz} (the
...
\subsection{Using extended precision arithmetic} \label{sec:ex1}
...
\subsection{Computing first order variational derivative} \label{sec:ex_varieq}
...
first variational equations. Again, for simplicity we use the
generic jet library instead of the specific (and more efficient) 
\texttt{jet\_1} library (see Section~\ref{sec:clo}).
We also add a trivial equation to
...
modify \odecfile{}. Append to the line
\begin{verbatim}
    extern MY_FLOAT RR;
\end{verbatim}
with 
...
\taylorname{} can be used to study stability of equilibrium points of
ODEs by computing its Jacobian and even bifurcations by computing
...
\item {\tt step\symbol{95}ctrl\symbol{95}method}\newline flag for the
  ...
  \item[-1:] user defined stepsize control. The code has to be
    ...
    \begin{center}
     {\tt compute\symbol{95}timestep\symbol{95}user\symbol{95}defined}
    \end{center}
    (see the code). The user must also include code for the selection
    of degree, in the function 
    \begin{center}
     {\tt compute\symbol{95}order\symbol{95}user\symbol{95}defined}.
    \end{center}
  \end{itemize}
\item {\tt log10abserr}\newline
...
\item {\tt endtime}
 \begin{description}
  \item if \verb+NULL+, it is ignored. \item if {\tt
    step\symbol{95}ctrl\symbol{95}method} is set to 0, it is also
    ignored.
  \item otherwise, if next step is going to be outside {\tt endtime},
    reduce the step size so that the new time {\tt time} is exactly
    {\tt endtime} (in that case, the function returns 1).
  \end{description}
\item {\tt ht}
 \begin{description}
  \item if \verb+NULL+, it is ignored. 
  \item on input: ignored/used as a time step (see parameter {\tt
  step\symbol{95}ctl\symbol{95}method})
  \item on output: time step used if the pointer is not \verb+NULL+.
  \end{description}
\item {\tt order}
 \begin{description}
  \item if \verb+NULL+, it is ignored. 
  \item on input: this parameter is only used if {\tt
  ...
  \item on output: degree used if the pointer is not \verb+NULL+.
  \end{description}
\item {\tt jetInOut}
\begin{description}
\item if \verb+NULL+, it is ignored. 
\item on input: the value of jet variables
\item on output: the new value of jet variable at the output time {\tt
  ti}.
\end{description}
\end{itemize}
...
\begin{command}
   ...
   taylor -name lorenz -o \odehfile{} -header \inputfile{}
\end{command}
...
\bibitem{Haetal16} A. Haro, M. Canadell, J.-Ll. Figueras, A. Luque
